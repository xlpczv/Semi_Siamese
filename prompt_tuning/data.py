...
## MS MARCO
def iter_marco_train_pairs(model, train_fp, batch_size):
    batch = {'query_id': [], 'doc_id': [], 'query_tok': [], 'doc_tok': []}
    for qid, did, query_tok, doc_tok in _iter_marco_train_pairs(model, train_fp):
        batch['query_id'].append(qid)
        batch['doc_id'].append(did)
        batch['query_tok'].append(query_tok)
        batch['doc_tok'].append(doc_tok)
...
def _iter_marco_train_pairs(model, train_fp):
    for line in train_fp:
        psd = line.rstrip().split('\t')
        if(len(psd) < 6): continue
        qid = psd[0]
        query = psd[1]
        pos_id = psd[2]
        pos_doc = psd[3]
