...
class BertRanker(torch.nn.Module):
    def __init__(self, lora_attn_dim=0, lora_attn_alpha=32, lora_dropout=0.0, lora_d_attn_dim=0, lora_d_attn_alpha=32, lora_d_dropout=0.0, without_bert=False, bert_model=None):
        super().__init__()
        ...
        if "bert-base" in self.BERT_MODEL:
            ...
            config = BertConfig(lora_attn_dim=lora_attn_dim, lora_attn_alpha=lora_attn_alpha, lora_dropout=lora_dropout,
                                lora_d_attn_dim=lora_d_attn_dim, lora_d_attn_alpha=lora_d_attn_alpha, lora_d_dropout=lora_d_dropout,
            ...
            config = BertConfig(hidden_size=1024, num_hidden_layers=24, num_attention_heads=16, intermediate_size=4096,
        ...
        if(without_bert):
            ...
            # print(BertModel(config=config))
            self.bert = BertModel.from_pretrained(self.BERT_MODEL, config=config)
    ...
    def freeze_bert(self):
        ...
        for n, p in self.bert.named_parameters():
            if 'adapter' in n:
                # print("adapter param", n)
                p.requires_grad = True
            else:
                # print("requires_grad False", n)
                p.requires_grad = False

    def get_params(self):
        params = [(k, v) for k, v in self.named_parameters() if v.requires_grad]
        lora_params = [v for k, v in params if 'adapter' in k]
        bert_params = [v for k, v in params if k.startswith('bert')]
        non_bert_params = [v for k, v in params if (not k.startswith('bert')) and ('rep' not in k)]
        prefix_params = [v for k, v in params if 'rep' in k]
        print("lora_params", [k for k, v in params if 'adapter' in k])
        print("bert_params", [k for k, v in params if k.startswith('bert')])
        print("non_bert_params", [k for k, v in params if (not k.startswith('bert')) and ('rep' not in k)])
        print("prefix_params", [k for k, v in params if 'rep' in k])
        return lora_params, bert_params, non_bert_params, prefix_params
    ...
    def encode_colbert(self, query_tok, query_mask, doc_tok, doc_mask, p_repr=None, p_start=0, device='cuda:0'):
...
class TwoBertRanker(torch.nn.Module):
    def __init__(self, lora_attn_dim=0, lora_attn_alpha=32, lora_dropout=0.0, lora_d_attn_dim=0, lora_d_attn_alpha=32, lora_d_dropout=0.0, without_bert=False, bert_model=None, asym=False, hetero_query=False, hetero_value=False):
        super().__init__()
        self.BERT_MODEL = bert_model
        self.hetero_query=hetero_query
    ...
    def encode_bert(self, query_tok, query_mask, doc_tok, doc_mask, p_qrepr=None, p_drepr=None, p_start=0):
        ...
        d_result_tuple = self.bert(d_toks, d_mask, d_segid.long(), prompt_hidden_states=p_drepr, prompt_hidden_start=p_start, qquery=self.hetero_query, vvalue=self.hetero_value)
    ...
    def encode_colbert(self, query_tok, query_mask, doc_tok, doc_mask, device='cuda:0', p_qrepr=None, p_drepr=None, p_start=0):
        ...
        # execute BERT model
        q_result_tuple = self.bert(q_toks, q_mask, q_segid.long(), prompt_hidden_states=p_qrepr)
        d_result_tuple = self.bert(d_toks, d_mask, d_segid.long(), prompt_hidden_states=p_drepr, qquery=self.hetero_query, vvalue=self.hetero_value)
        q_result = q_result_tuple[2]
        d_result = d_result_tuple[2]
...
class VanillaBertRanker(BertRanker):
    def __init__(self, without_bert=False, bert_model=None, lora_attn_dim=0, lora_attn_alpha=32, lora_dropout=0.0, lora_d_attn_dim=0, lora_d_attn_alpha=32, lora_d_dropout=0.0, p_start=0, hsize=0, hlen=0):
        print("lora_attn_dim^^", lora_attn_dim)
...
class TwinBertRanker(TwoBertRanker):
    def __init__(self, without_bert=False, bert_model=None, qd=True, asym=False, lora_attn_dim=0, lora_attn_alpha=32, lora_dropout=0.0, lora_d_attn_dim=0, lora_d_attn_alpha=32, lora_d_dropout=0.0, hetero_query=False, hetero_value=False, p_start=0, hsize=0, hlen=0):
...
class ColBertRanker(TwoBertRanker):
    def __init__(self, without_bert=False, bert_model=None, asym=False, lora_attn_dim=0, lora_attn_alpha=32, lora_dropout=0.0, lora_d_attn_dim=0, lora_d_attn_alpha=32, lora_d_dropout=0.0, hetero_query=False, hetero_value=False, p_start=0, hsize=0, hlen=0):
        super().__init__(without_bert=without_bert, bert_model=bert_model, asym=asym,
                        lora_attn_dim=lora_attn_dim, lora_attn_alpha=lora_attn_alpha, lora_dropout=lora_dropout,
                        lora_d_attn_dim=lora_d_attn_dim, lora_d_attn_alpha=lora_d_attn_alpha, lora_d_dropout=lora_d_dropout,
                        hetero_query=hetero_query,
    ...
    def forward(self, query_tok, query_mask, doc_tok, doc_mask, value_return=False):
        ...
        # 2) Query augmentation with [MASK] tokens ([MASK] = 103)
        if self.hlen> 0:
            q_cls_reps, d_cls_reps, q_reps, query_mask, d_reps, doc_mask = self.encode_colbert(query_tok, query_mask, doc_tok, doc_mask, 
            p_qrepr=theta,
            p_drepr=theta,
            p_start=self.p_start,
            )
        else:
            q_cls_reps, d_cls_reps, q_reps, query_mask, d_reps, doc_mask = self.encode_colbert(query_tok, query_mask, doc_tok, doc_mask)  # reps includes rep of [CLS], [SEP]
