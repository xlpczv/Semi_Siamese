...
def main(args, model, dataset, train_pairs, qrels, valid_run, qrelf):
    ...
    nonbert_params, bert_params = model.get_params(args)
    optim_nonbert_params = {'params': nonbert_params, 'lr': args.non_bert_lr}
    ...
    with tqdm('training', total=args.batch_size * args.batches_per_epoch, ncols=80, desc='train', leave=False) as pbar:
        for record in data.iter_train_pairs(model, dataset, train_pairs, qrels, args.grad_acc_size): 
            ...
            if total % args.batch_size == 0:
                ...
                # print("model.bert.encoder.layer[0].attention.self.query.weight", model.bert.encoder.layer[0].attention.self.query.weight)
            pbar.update(count)
...
def run_model(args, model, dataset, run, runf, desc='valid'):
    ...
    with torch.no_grad(), tqdm(total=sum(len(r) for r in run.values()), ncols=80, desc=desc, leave=False) as pbar:
        ...
        for records in data.iter_valid_records(model, dataset, run, args.batch_size):
            # print(records['query_tok'].shape, records['query_mask'].shape, records['doc_tok'].shape, records['doc_mask'].shape)
            scores = model(records['query_tok'],
                           records['query_mask'],
                           records['doc_tok'],
                           records['doc_mask'])
            # print("scores.shape", scores.shape) ## [bs, 1]
            for qid, did, score in zip(records['query_id'], records['doc_id'], scores):
                # print("qid", qid, "did", did, "score", score)
...
def main_cli():
    ...
    parser.add_argument('--asym', type=bool, default=False, help='whether to assymetric bert models or not')
    ...
    if args.asym:
        model = MODEL_MAP[args.model](bert_model=args.model_card, asym=True).cuda()
    else:
        model = MODEL_MAP[args.model](bert_model=args.model_card).cuda()
